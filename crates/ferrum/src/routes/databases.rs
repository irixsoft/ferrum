...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/postgres/extensions", get(extensions))
...
/// One psql query, so it is its own route rather than part of the polled status.
async fn extensions(Extract(app): Extract<AppState>, _: Caller) -> ApiResult<Json<Vec<String>>> {
    if app.platform.postgres_major_installed().is_none() {
        return Err(ApiError::conflict(NOT_INSTALLED));
    }
    let names = postgres::available(app.platform.as_ref()).map_err(|e| db_error(e.into()))?;
    Ok(Json(names))
}
...
/// Creates `{name}` from the uploaded dump: the dump is streamed to disk and refused early —
/// five bytes for a gzip, the first byte past the free space — then read for the extensions it
/// needs, which the superuser turns on with the picked ones before the load runs as the role.
async fn create_from_dump(
    Extract(app): Extract<AppState>,
    _: Caller,
    Path(name): Path<String>,
    axum::extract::Query(params): axum::extract::Query<FromDump>,
    request: Request,
) -> ApiResult<(StatusCode, Json<serde_json::Value>)> {
    if app.platform.postgres_major_installed().is_none() {
        return Err(ApiError::conflict(NOT_INSTALLED));
    }
    if !postgres::valid_name(&name) {
        return Err(ApiError::bad_request(
            "A database name is 1 to 63 characters of lowercase letters, digits and underscores, starting with a letter.",
        ));
    }
    if postgres::by_name(&app.db, &name).await?.is_some() {
    ...
    let role = name.clone();
    let (staged, prepared) = tokio::task::spawn_blocking(move || {
        let prepared = restore::prepare(platform.as_ref(), &staged, format, &role);
        (staged, prepared)
    })
    .await
    ...
    let mut extensions: Vec<String> = params
        ...
        .map(String::from)
        .collect();
    for ext in prepared.extensions {
