...
impl Platform for FakePlatform {
    ...
    fn postgres_restore_list(&self, from: &Path) -> Result<String, PlatformError> {
        self.record(format!("postgres_restore_list {}", from.to_string_lossy()))?;
        Ok(self.inner.lock().unwrap().restore_list.clone())
    }
    ...
    fn postgres_restore(
        ...
        self.record(format!(
            ...
            role.unwrap_or("-"),
            list.map_or("-".to_string(), |l| l.to_string_lossy().to_string())
        ))
    }
