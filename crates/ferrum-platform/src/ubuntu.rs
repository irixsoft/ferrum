...
impl Platform for Ubuntu {
    ...
    fn postgres_restore_list(&self, from: &Path) -> Result<String, PlatformError> {
        ...
        exec::run(&["runuser", "-u", PG_USER, "--", "pg_restore", "-l", &from])
    }
    ...
    fn postgres_restore(
        ...
        let list = list.map(|l| l.to_string_lossy().to_string());
        let mut argv = vec![
            "runuser",
            "-u",
            PG_USER,
        ...
        if let Some(role) = &role {
            argv.extend(["--no-owner", "--no-privileges", role]);
        }
        if let Some(list) = &list {
            argv.extend(["-L", list]);
        }
        argv.extend(["-d", database, &from]);
        exec::run(&argv).map(|_| ())
    }
