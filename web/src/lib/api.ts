...
/** What the server can CREATE EXTENSION, plus vector, which Ferrum installs a package for. */
export function useExtensions() {
  return useQuery({
    queryKey: keys.extensions,
    queryFn: () => request<string[]>("/postgres/extensions"),
    staleTime: 60_000,
  });
}
...
export function createFromDump({
  name,
  file,
  connection_limit,
  app_slug,
  extensions,
  onProgress,
}: FromDump): Promise<Database> {
  const query = new URLSearchParams();
  if (connection_limit !== undefined) query.set("connection_limit", String(connection_limit));
  if (app_slug) query.set("app_slug", app_slug);
  if (extensions?.length) query.set("extensions", extensions.join(","));
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open("POST", `/api/databases/${name}/restore?${query}`);
...
export function useCreateFromDump() {
  return useInvalidating([keys.databases, keys.apps], createFromDump);
}
